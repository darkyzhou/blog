...
export const title = '绝对布局的“现代接任者”？——记 CSS Grid 布局的一个妙用'
export const date = '2021-03-02T00:25:29.124Z'
...
<BlogImage filePath="one-css-grid-trick/one-css-grid-trick-1" />
...
通过这样，我们的确能够得到想要的结果：
<BlogImage filePath="one-css-grid-trick/one-css-grid-trick-2" />
...
<BlogImage filePath="one-css-grid-trick/one-css-grid-trick-3" />
...
<BlogImage filePath="one-css-grid-trick/one-css-grid-trick-4" />

在上面的例子里，中间的文字使用了下面的样式（有删减）：

```css
.smol-stack-layout h3 {
  /* 这是 align-self 和 justify-self 的缩写属性 */
...
.smol-stack-layout small {
  /* align-self 对应在 Block axis（在这里是纵轴）上的对齐方式 */
  align-self: end;
  /* justify-self 对应在 Inline Axis（在这里是横轴）上的对齐方式 */
  justify-self: start;
}
```
...
<BlogImage filePath="one-css-grid-trick/one-css-grid-trick-7" />
...
<BlogImage filePath="one-css-grid-trick/one-css-grid-trick-5" />

<BlogImage filePath="one-css-grid-trick/one-css-grid-trick-6" />
...
- [https://smolcss.dev/#smol-stack-layout](https://smolcss.dev/#smol-stack-layout)
