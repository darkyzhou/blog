export const title = '从 package.json 和 package-lock.json 浅谈 npm 解析依赖出错问题'
export const date = '2020-06-26T03:01:52.123Z'
...
`npm install` 是 Node.js 世界中使用频率最高的命令之一。这个命令会根据 `package.json` 的内容，将项目所依赖的库安装到本地的 `node_modules` 文件中，然后产生一个新文件 `package-lock.json`。
