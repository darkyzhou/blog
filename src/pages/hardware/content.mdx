...
- RAM: SAMSUNG 4Rx4 DDR4-2400 64 GiB x 8
- GPU: Inno3D RTX 5060 Ti TWIN X2 16 GB
- SSD: WD SN7100 2 TiB
- Motherboard: HUANANZHI H12D-8D
- OS: NixOS 25.11 (Xantusia) x86_64

这台主机主要用于日常的 Linux 开发。

> 这其实是我的第七代 All in One 主机，前五代的 CPU 分别是：
...
> - 第六代：Intel Core i7-12700T，服役时间 2024.9-2025.5。

我的第六代 All in One 主机原本安装的是 Proxmox VE，后来我把其中跑各种服务的 VM 搬迁到了下面的 sirin 上。此时它只剩下用于日常开发的 VM 了，我便将它重装为 NixOS。由此，它已经不能被称作 All in One 主机了。有趣的是，纵观我的前六代 All in One 主机，可以发现一个趋势：承担不同职能的 VM 被不断地拆分到其他主机上，包括软路由、NAS 等职能，这可能是所谓的「合久必分」吧。
...
<BlogImage filePath='hardware/thehub' caption="第六代主机，装机时拍的图，其中的电源购于京东自营，但插电后发现是坏的，我直接退货了" />

上图中还有一张蓝色外壳的显卡，那是下面会专门介绍的 AMD Radeon Pro WX 4100，现在它已经被我换到下面的 herta 主机上了。

不过，在最新的第七代中，我又重新装上了 Proxmox VE 系统，因为这台主机其实是我和舍友共同购买的，需要使用虚拟化技术来贡献资源。这台主机上还有一张 RTX 5060 Ti 显卡，我会用它来跑一些 AI 服务，例如我自己设计的基于 LLM 的智能音箱。

<BlogImage filePath='hardware/epyc' caption="第七代主机，装机时拍的图" />
<BlogImage filePath='hardware/epyc2' caption="第七代主机，装好后拍的图" />

在组装这台主机的过程中，我遇到了不少意外。最开始安装 Proxmox VE 时，安装程序总是在某个环节直接重启。我排查了很久，怀疑是硬盘有问题，更换后顺利安装，但舍友后来测试那块硬盘却说没问题。接下来，我把主板装进机箱开机，发现 BIOS 只识别到七根内存。反复拆装后才发现，机箱上多出来的主板螺柱正好顶住了出问题内存槽背部的引脚。让我感叹现代硬件的鲁棒性，居然没有因此短路烧毁。

之后，我在 Proxmox VE 上跑编译测试时，系统又频繁崩溃重启。查日志发现总是 CPU 32 和 96 报错，且它们属于同一个物理核心。我推测是 CPU 硬件故障，联系商家更换后问题解决。

<BlogImage filePath='hardware/epyc3' />

等我终于开始安装 NixOS 虚拟机时，又遇到虚拟机启动导致系统重启的问题。我排查后发现，WD SN7100 固态硬盘通过 PCIe 直通给虚拟机后，无法正确响应 VFIO 的 reset 命令，系统直接重启。只能改用 QEMU 的软件直通方式。后续在 NixOS 虚拟机配置好 RTX 5060 Ti 显卡直通后，发现虚拟机重启会导致显卡失效。最后我是通过升级 Linux 内核到刚发布的 6.15.0、手动编译最新 NVIDIA 驱动和更新显卡 VBIOS 才解决的。

## timido

- CPU: AMD Ryzen 9 9800X3D (8 cores, 16 threads)
- RAM: ADATA XPG LANCER BLADE DDR5-6000 32 GiB x 2
- GPU: GIGABYTE RTX 5090 WINDFORCE
...
<BlogImage filePath="hardware/timido3" caption="它现在位于我的电视旁，机箱是九州风神的 CH270" />
...
这台主机就是前文所说的，在我的 All in One 主机被拆分后，用于跑各种服务的主机。N100 自带的核显的视频编解码性能不错，可以用于 Jellyfin 串流。它运行的其它服务包括 qBittorrent、nginx、[vaultwarden](https://github.com/dani-garcia/vaultwarden) 等。这台主机使用了 [XanMod 内核](https://xanmod.org/)，它在原版内核的基础上添加了一些补丁来提高系统的稳定性、响应速度等（尽管感觉效果不明显）。
